...
from ivs_derived_zero_trip import derived as counter, run

for apply_ivs in (False, True):
    sdfg = counter.to_sdfg(simplify=True)
    sdfg.name = f'derived_iso_{int(apply_ivs)}'
    ...
    print('IVS applied' if apply_ivs else 'no IVS     ', 'N=0 out =', run(sdfg, 0), '(expected 100)')
