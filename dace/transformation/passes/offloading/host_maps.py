...
def maps_pinned_by_host_loops(sdfg: SDFG) -> OrderedSet:
    ...
    for loop in sdfg.all_control_flow_regions():
        if not isinstance(loop, LoopRegion):
            continue
        # Only a pin directly in a guard's branch is a fallback; a carried-dependence pin is not.
        if loop.pinned_sequential and isinstance(loop.parent_graph.parent_graph, ConditionalBlock):
