...
def symbol_guarded_if_else(guard: str, before: dict[str, str], after: dict[str, str]) -> dace.SDFG:
    """``<before>; if <guard>: A[0] = 1.0 else: A[0] = 2.0; <after>``, with ``k`` an int64 argument."""
    sdfg = dace.SDFG(f"guard_{guard[0]}")
    sdfg.add_array("A", shape=(1, ), dtype=dace.float64)
    sdfg.add_symbol("k", dace.int64)
    cb = ConditionalBlock("cb")
    sdfg.add_edge(sdfg.add_state("entry", is_start_block=True), cb, dace.InterstateEdge(assignments=before))
    ...
    for cond, value in ((CodeBlock(guard), "1.0"), (None, "2.0")):
        ...
        state = arm.add_state(is_start_block=True)
        state.add_edge(state.add_tasklet("set", {}, {"_a"}, f"_a = {value}"), "_a", state.add_write("A"), None,
                       dace.Memlet("A[0]"))
...
def test_guard_symbol_rebound_after_the_branch_is_tested_at_its_incoming_value():
    sdfg = symbol_guarded_if_else("k > 0", {}, {"k": "k - 1"})
    A = np.zeros(1)

    SameWriteSetIfElseToITECFG().apply_pass(sdfg, {})

    assert "_cond_k" not in sdfg.arrays
    sdfg(A=A, k=1)
    assert A[0] == 1.0


def test_lifted_guard_symbol_bound_to_a_value_keeps_the_symbol_type():
    sdfg = symbol_guarded_if_else("m > 1", {"m": "k - 1"}, {})
    A = np.zeros(1)
