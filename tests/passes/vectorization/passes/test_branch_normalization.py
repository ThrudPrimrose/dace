...
def test_hoist_ahead_of_a_refused_lowering_is_reported_as_a_change():
    ...
    cb = ConditionalBlock("cb")
    sdfg.add_edge(sdfg.add_state("init", is_start_block=True), cb, dace.InterstateEdge())
    arm = ControlFlowRegion("arm", sdfg=sdfg)
    body = arm.add_state("body")
    arm.add_edge(arm.add_state("entry", is_start_block=True), body, dace.InterstateEdge(assignments={"__sym_z1": "z1"}))
    body.add_mapped_tasklet("m", {"j": "0:8"}, {}, "_out = __sym_z1", {"_out": dace.Memlet("b[j]")}, external_edges=True)
    cb.add_branch(CodeBlock("z1 > 0"), arm)
